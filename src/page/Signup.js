    ...
    const handleSubmit = async(e) => {
        ...
        if(firstName && email && password && confirmpassword){
            if(password === confirmpassword){
                ...
                alert("Check password and Confirm password is equal or not..!")
            }
        }else{
    ...
    return (
        <div className="p-3 md:py-8">
            <div className="w-full max-w-sm bg-white m-auto flex-col p-4">
                ...
                <p className="flex items-center text-sm -mt-2">Already have an account ? <Link to={"/login"} className="text-red-500 underline">Login</Link></p>
            </div>
        </div>
